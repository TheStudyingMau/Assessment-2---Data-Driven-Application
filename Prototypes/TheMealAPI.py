...
class ErrorDisplay(): # Responsible for informing user their error for searchGUI.
    ...
    def __init__(self, parent):
        self.root = parent # Stores the root in itself for its labels.
    
    def noitemsfound(self): # Method that displays 'No Items Found.'
        self.label = ctk.CTkLabel(self.root.display1, text="", font=p) # Label is constructed.
...
class TkinterGUI(ctk.CTk): # Handles the main GUI elements.
...
GUI = TkinterGUI() # Instantiates the TkinterGUI class.
SYS = ErrorDisplay(GUI) # Instantiates the ErrorDisplay class.
